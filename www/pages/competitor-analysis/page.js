...
const Page = {
  async init() {
    ...
    const benchmarkRange = document.getElementById('ca_benchmark_range');

    const TASK = 'competitor_analysis';
    const MODELS = [
      { id: 'doubao', name: '豆包' },
      { id: 'yuanbao', name: '元宝' },
    ...
    const prefillFields = [
      { id: 'ca_enterprise_full_name', kbKey: '企业全称' },
      { id: 'ca_enterprise_full_name_dup', kbKey: '企业全称' },
      { id: 'ca_enterprise_website', kbKey: '企业官网' },
      { id: 'ca_main_products', kbKey: '主营产品' },
      { id: 'ca_sales_region', kbKey: '销售区域范围' },
      { id: 'ca_enterprise_advantage', kbKey: '企业优势' },
    ];
    ...
    const buildPageContext = (fields) => {
      ...
      const range = String(benchmarkRange?.value || '').trim();
      if (range) lines.push(`- 想要对标范围：${range}`);
      return lines.join('\n');
    };

    const fn = document.getElementById('ca_enterprise_full_name');
    const fnDup = document.getElementById('ca_enterprise_full_name_dup');
    ...
    if (benchmarkRange instanceof HTMLInputElement) {
      benchmarkRange.classList.add('manual-input');
    }
    ...
    const runFourModels = async () => {
      ...
      for (const m of MODELS) {
        const hint = `请按照${m.name}大模型的风格生成内容。\n\n`;
        const req_id = `ca_${m.id}_${Date.now()}_${Math.random().toString(16).slice(2)}`;
        window.geoAiExecute?.({ task: TASK, req_id, competitors: `${hint}${base}`, page_context });
    ...
    const runSummary = async () => {
      ...
      const req_id = `ca_summary_${Date.now()}_${Math.random().toString(16).slice(2)}`;
      const prompt =
        "请把下面多个大模型的竞争对手分析内容综合起来，去重汇总，生成一篇新的文章（结构清晰，可直接发布）。\n" +
        "要求：标题+正文；要点分段；尽量引用具体结论；避免重复。\n\n" +
        combined;
      window.geoAiExecute?.({ task: TASK, req_id, competitors: prompt, page_context });
      const text = await waitForAi(req_id);
