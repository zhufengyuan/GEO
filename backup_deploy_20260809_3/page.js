...
const Page = {
  async init() {
    ...
    document.getElementById('genTimelineBtn')?.addEventListener('click', () => {
      ...
      window.geoAiExecute?.({ task: 'generate_kb_timeline', req_id, kb_form_values: kbFormValues });
    ...
    document.getElementById('genProfileBtn')?.addEventListener('click', () => {
      ...
      if (el) el.value = '生成中...';
      const kbFormValues = collectBasePayload()?.data || {};
      window.geoAiExecute?.({ task: 'generate_kb_profile', req_id, kb_form_values: kbFormValues });
    ...
    document.getElementById('genLibraryBtn')?.addEventListener('click', () => {
      ...
      if (el) el.value = '生成中...';
      const kbFormValues = collectBasePayload()?.data || {};
      window.geoAiExecute?.({ task: 'generate_kb_library', req_id, kb_form_values: kbFormValues });
    ...
    const fillProducts = (data) => {
      if (!data || typeof data !== 'object') return;
      if (Array.isArray(data.rows)) renderProducts(data.rows); else if (Array.isArray(data.products)) renderProducts(data.products);
    };
    ...
    const SECTIONS = [
      ...
      { title: '客户与市场', keys: ['目标客群', '市场布局', '服务行业'] },
